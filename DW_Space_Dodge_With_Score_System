...
# Initialize semi-transparent surface for pause tint effect
PAUSE_ALPHA = 100  # Change the alpha value as needed
PAUSE_SURFACE = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)
PAUSE_SURFACE.fill((0, 0, 0, PAUSE_ALPHA))

# Pause delay constants
PAUSE_DELAY = 1000  # 1000 milliseconds = 1 second



# Function to draw objects on the game window
def draw(player, projectiles, elapsed_time, stars, score, paused):
    global bg_y
    WIN.blit(BG, (0, bg_y))
    ...
    # Render and display the asteroid score
    score_text = FONT.render(f"Score: {score}", 1, "white")
    WIN.blit(score_text, (10, 75))
    ...
    if paused:
        WIN.blit(PAUSE_SURFACE, (0, 0))  # Overlay the pause tint surface
...
def main():
    ...
    while run:
        ...
        if paused: 
            pygame.time.delay(100) == True # Add a small delay to avoid high CPU usage during pause
        ...
        draw(player, projectiles, elapsed_time, stars, score, paused)
